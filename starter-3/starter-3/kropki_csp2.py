...
def kropki_csp_model_1(initial_kropki_board):
    ...
    '''
    double for loop with range of the dimision of board and you have to check all the cells
    it is a dimxdim matrix
    so then check if the cell what the cells are and make a variable and assign them the cell
    
    so basically you need to check every cell (use the pychar debugger to see what variables mean what)
    
     append the rows and col and the variable_array with the variables
     ...
     Now deal with boxes, same set up as rows and col
     then fill up the boxes based on the box descrpt
    box_w = d//3
    box_h = 3
    ...
    this is for the tuples
    
    Now for rows, cols, and boxes find all the permutations and check them with a for loop
    check the scope and add the constrains (need to use the constrain data type for this in cspbase)
    add all constrains to the csp
    
    Now find all the tuples that are consectuive or double
    use the board attributes to check if theyre consectuve rows, col, double row, double col, create those
    constrains and add them to the csp, there should be four cases.
    board.consec_row[x][y]
    '''
